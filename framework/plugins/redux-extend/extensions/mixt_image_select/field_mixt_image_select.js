...
(function( $ ) {
    ...
    redux.field_objects.mixt_image_select = redux.field_objects.mixt_image_select || {};

    $( document ).ready(
        function() {
            //redux.field_objects.mixt_image_select.init();
        }
    );

    redux.field_objects.mixt_image_select.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( '.redux-container-mixt_image_select' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                el.find( '.redux-mixt-image-select label img, .redux-mixt-image-select label .tiles' ).click(
                    function( e ) {
                        var id = $( this ).closest( 'label' ).attr( 'for' );

                        $( this ).parents( "fieldset:first" ).find( '.redux-mixt-image-select-selected' ).removeClass( 'redux-mixt-image-select-selected' ).find( "input[type='radio']" ).attr(
                            "checked", false
                        );
                        $( this ).closest( 'label' ).find( 'input[type="radio"]' ).prop( 'checked' );

                        if ( $( this ).closest( 'label' ).hasClass( 'redux-mixt-image-select-preset-' + id ) ) { // If they clicked on a preset, import!
                            e.preventDefault();
                            ...
                            if ( presets !== undefined && presets !== null ) {
                                ...
                                if ( answer ) {
                                    el.find( 'label[for="' + id + '"]' ).addClass( 'redux-mixt-image-select-selected' ).find( "input[type='radio']" ).attr(
                                        "checked", true
                                    );
