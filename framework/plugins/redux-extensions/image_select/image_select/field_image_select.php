...
if ( ! class_exists( 'ReduxFramework_image_select' ) ) {
	...
	class ReduxFramework_image_select {
		...
		public function render() {
			...
			if ( ! empty( $this->field['options'] ) ) {
				...
				foreach ( $this->field['options'] as $k => $v ) {
					...
					if ( strpos($this->field['class'], 'noUpdate') === false ) {
					...
					echo '<input type="radio" class="' . $this->field['class'] . '" id="' . $this->field['id'] . '_' . ( array_search( $k, array_keys( $this->field['options'] ) ) + 1 ) . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '" value="' . $input_val . '" ' . checked( $this->value, $theValue, false ) . $presets . '/>';
					if ( ! empty( $this->field['tiles'] ) && $this->field['tiles'] == true ) {
						echo '<span class="tiles" style="background-image: url(' . $v['img'] . ');" rel="' . $v['img'] . '"">&nbsp;</span>';
					} else {
						echo '<img src="' . $v['img'] . '" alt="' . $v['alt'] . '" title="' . $v['alt'] . '" style="' . $style . '"' . $presets . ' />';
					}
		...
		public function enqueue() {
			...
			$file_url = MIXT_PLUGINS_URI . '/redux-extensions/image_select/image_select';

			wp_enqueue_script(
				'redux-field-image-select-js',
				$file_url . '/field_image_select' . Redux_Functions::isMin() . '.js',
